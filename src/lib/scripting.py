...
class Keyboard:
    ...
    def set_interval(self,interval=0.0): 
        """
        Sets the interval between keystrokes for send_keys
        
        Usage: C{keyboard.set_interval(interval=0.0)}
        
        Some applications do not respond correctly to strings when
        sent rapidly.  Setting 'interval' will insert a delay of 'interval' between
        each keystroke.
        @param interval: wait this long between each keystroke in seconds
        """
        self.interval = interval
        
    def set_method(self,method='event'): 
        """
        Sets the method used to simulate keystrokes.
        
        Usage: C{keyboard.set_method(method='event')}

        Currently supported methods are 'xtest' and 'event'
        Note: Some keyboard events are not supported by 'event' and will
        still be sent using 'xtest' regardless of this setting
        
        @param method: method to utilize to send events 
        """
        self.method = method
    
    def grab_keyboard(self):
        """
        Block user input from the keyboard
        
        Usage: C{keyboard.grab_keyboard()}
        
        """
        self.mediator.interface.grab_keyboard()

    def ungrab_keyboard(self):
        """
        Un-block user input from the keyboard
        
        Usage: C{keyboard.ungrab_keyboard()}
        
        """
        self.mediator.interface.ungrab_keyboard()

    def send_keys(self, keyString,interval=0.0):
        """
        Send a sequence of keys via keyboard events
        
        Usage: C{keyboard.send_keys(keyString, interval=0)}
        
        @param keyString: string of keys (including special keys) to send
        @param interval: when set, overrides interval specified with C{keyboard.set_interval(interval=0.0)}
        """
        if interval:
            self.mediator.send_string(keyString.decode("utf-8"), interval, self.method)
        else:
            self.mediator.send_string(keyString.decode("utf-8"), self.interval, self.method)
    ...
    def send_key(self, key, repeat=1):
        ...
        for x in xrange(repeat):
            self.mediator.send_key(key.decode("utf-8"),self.method)
        self.mediator.flush()
    ...
    def press_key(self, key):
        ...
        These events are always sent using XTest.
...
class Mouse:
    ...
    def grab_mouse(self):
        """
        Block user input from the mouse
        
        Usage: C{mouse.grab_mouse()}
        
        """
        self.mediator.interface.grab_pointer()

    def ungrab_mouse(self):
        """
        Un-block user input from the mouse
        
        Usage: C{mouse.ungrab_mouse()}
        
        """
        self.mediator.interface.ungrab_pointer()
